...
import org.lineageos.settings.device.camera.CameraUtils;
...
public class BootReceiver extends BroadcastReceiver implements Utils {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        FileUtils.setValue(DeviceSettings.FLASH0_BRIGHTNESS_PATH, Settings.Secure.getInt(context.getContentResolver(), DeviceSettings.PREF_FLASH0_BRIGHTNESS, 120));
        FileUtils.setValue(DeviceSettings.FLASH1_BRIGHTNESS_PATH, Settings.Secure.getInt(context.getContentResolver(), DeviceSettings.PREF_FLASH1_BRIGHTNESS, 120));
        //int gain = Settings.Secure.getInt(context.getContentResolver(), DeviceSettings.PREF_HEADPHONE_GAIN, 0);
        //FileUtils.setValue(HEADPHONE_GAIN_PATH, gain + " " + gain);
        //FileUtils.setValue(MIC_GAIN_PATH, Settings.Secure.getInt(context.getContentResolver(), DeviceSettings.PREF_MIC_GAIN, 0));
        FileUtils.setValue(DeviceSettings.NOTIF_LED_PATH,(1 + Math.pow(1.05694, Settings.Secure.getInt(context.getContentResolver(), DeviceSettings.PREF_NOTIF_LED, 100))));
        FileUtils.setValue(DeviceSettings.VIBRATION_STRENGTH_PATH, Settings.Secure.getInt(context.getContentResolver(), DeviceSettings.PREF_VIBRATION_STRENGTH, 20) / 100.0 * (DeviceSettings.MAX_VIBRATION - DeviceSettings.MIN_VIBRATION)+ DeviceSettings.MIN_VIBRATION);
